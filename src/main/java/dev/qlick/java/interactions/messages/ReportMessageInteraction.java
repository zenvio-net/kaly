package dev.qlick.java.interactions.messages;
...
public class ReportMessageInteraction extends ListenerAdapter {
    ...
    public void onMessageContextInteraction(MessageContextInteractionEvent event) {
        ...
        TextInput reason = TextInput.create("report-message-reason", "Reason", TextInputStyle.SHORT)
                .setMinLength(5)
                .setMaxLength(500)
                .setRequired(true)
                .build();
        ...
        event.replyModal(Modal.create(
                String.format("report-message-modal-%s-%s-%s-%s", target.getId(), message.getGuild().getId(), message.getChannel().getId(), message.getId()),
    ...
    public void onModalInteraction(ModalInteractionEvent event) {
        if (!event.getModalId().startsWith("report-message-modal-")) return;

        String[] splitModalId = event.getModalId().split("-");
        ...
        MessageEmbed embed = new EmbedBuilder()
                ...
                .addField("Reason", event.getValue("report-message-reason").getAsString(), true)
